...
des = input("with epsilon yes or no :")
if des == "no":
    nfa = {}
    n = int(input("No. of states : "))  # Enter total no. of states
    t = int(input("No. of symbol : "))  # Enter total no. of transitions/paths eg: a,b so input 2 for a,b,c input 3
    for i in range(n):
    ...
    for ke in key :
        ...
        for i in k :
            c = x[i]
            for j in c :
                G.add_edge(ke, j, weight=i)

    pos = nx.spring_layout(G)
    nx.draw(G, pos, with_labels=True, node_size=1000, node_color='yellow')
    nx.draw_networkx_edge_labels(G, pos, font_size=5, edge_labels=nx.get_edge_attributes(G, 'weight'))
    plt.show()'''
    ...
    while True:
        li = []
        start = dfa_states_list[0]
        q = input("Enter the string to check..\n")
        for i in q:
            li.append(dfa[start][i])
            print("-- {} read {} go to {}".format(start, i, dfa[start][i]))
            start = dfa[start][i]
        if (li[-1] in dfa_final_states):
            print("yes")
        else:
             print("no")
